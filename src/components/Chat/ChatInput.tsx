  ...
  return (
    <div className="relative">
      ...
      <button onClick={() => handleSend()} disabled={!!loading}>
        <IconArrowUp className={`absolute right-2 bottom-3 h-8 w-8 rounded-full p-1 bg-blue-500 text-white hover:opacity-80 ${loading ? 'opacity-50 cursor-not-allowed' : 'hover:cursor-pointer'}`} />
      </button>
    </div>
  );
};
